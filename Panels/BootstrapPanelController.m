 ...
 *
 *	BootstrapPanelController.m - Controller for the bootstrap panel
 *
...
#define BRACKET_IMAGE		@"bracket.png"
#define BRACKET_DARK_IMAGE	@"bracketDark.png"
...
- (void) awakeFromNib
{
	...
	[[NSDistributedNotificationCenter defaultCenter] addObserver:self
		selector:@selector(notifyUpdateBracket:) name:THEME_CHANGED_NOTIFICATION object:nil];
	[self notifyUpdateBracket:nil];
#endif
}


- (void) notifyUpdateBracket:(NSNotification *)notification
{
	[bracketView setImage:[NSImage imageNamed:isMojaveDarkModeActive() ? BRACKET_DARK_IMAGE : BRACKET_IMAGE]];
}
...
- (IBAction) loadBootstrapLoader:(id)sender
{
	...
	error = [pdp8 loadPaperTape:path toField:(ushort) field];
	if (error) {
		if (! path)
		...
		[pdp8 setPC:(ushort) addr];
		[pdp8 setIF:(ushort) field];
		[pdp8 setIB:(ushort) field];
	}
}
