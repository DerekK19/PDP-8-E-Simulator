...
- (BOOL) canDragRowsWithIndexes:(NSIndexSet *)rows atPoint:(NSPoint)mouseDownPoint
{
    ...
    // DGK Commented out for ARC 30/5/18
...
{
    CPUMemoryViewController *viewer = [self delegate];
    ...
    if (([event modifierFlags] & NSEventModifierFlagShift) &&
        [self columnAtPoint:point] == OPCODE_COLUMN &&
        (toolTipContent = [viewer operandInfoAtAddress:row])) {
        NSRect rect = [self frameOfCellAtColumn:OPCODE_COLUMN row:row];
        if (tooltipTag != 0) {
            [self removeToolTip:tooltipTag];
            tooltipTag = 0;
        }
        tooltipTag =  [self addToolTipRect:rect owner:toolTipContent userData:nil];
    } else {
        if (tooltipTag != 0) {
            [self removeToolTip:tooltipTag];
            tooltipTag = 0;
...
- (void) setYosemiteTitleKerning:(NSCell *)cell
{
    ...
    if (runningOnYosemiteOrNewer()) {
        ...
        if (runningOnElCapitanOrNewer())
            kern = -0.5;
        [cell setAttributedStringValue:[[NSAttributedString alloc] initWithString:[cell title] attributes:
...
- (NSString *) operandInfoAtAddress:(NSInteger)addr
{
    return [[Disassembler sharedDisassembler] operandInfoForPDP8:pdp8 atAddress:(int)addr];
}
